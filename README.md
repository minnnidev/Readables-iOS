...
| `SnapKit`, `Then` | 코드베이스로 UI 코드를 작성하여 협업 시 원활학[ 레이아웃을  설정하였습니다. |
...
#### 2. Authentication
   - 애플 로그인과 카카오 로그인을 연동하고, 필요한 정보를 서비스 서버로 보내 회원 인증을 진행합니다. 현재 계정 연동 상태에 따라 자동 로그인 여부를 결정하고, 도달할 화면에 대한 분기 처리를 실시하였습니다.
